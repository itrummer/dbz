...
def logical_and(column_1, column_2):
    ...
    """
    return [column_1[i] and column_2[i] for i in range(len(column_1))]
...
def logical_or(column_1, column_2):
    ...
    """
    return [column_1[i] or column_2[i] for i in range(len(column_1))]
...
def group_by_min(table, agg_column, group_columns):
    ...
    for row in table:
        ...
        elif row[agg_column] < group_dict[group_tuple]:
            group_dict[group_tuple] = row[agg_column]
...
def group_by_max(table, agg_column, group_columns):
    ...
    for row in table:
        ...
        if group_tuple not in group_dict:
            group_dict[group_tuple] = 0
        # add agg_column value to group_dict
        group_dict[group_tuple] = max(group_dict[group_tuple], row[agg_column])
...
def group_by_avg(table, agg_column, group_columns):
    ...
    group_list = []
    for key in group_dict:
        group_list.append(list(key) + [group_dict[key][0] / group_dict[key][1]])
    return group_list
...
def full_outer_join(table_1, table_2, key_cols_1, key_cols_2):
    ...
    for key, rows in table_1_dict.items():
        if key not in [tuple([row[i] for i in key_cols_2]) for row in table_2]:
            for row in rows:
                joined_rows.append(row + [None] * len(table_2[0]))
    return joined_rows
